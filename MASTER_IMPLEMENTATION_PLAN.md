...
| Repository / codebase | 🟡 In progress — Laravel backend scaffolded; register/login/logout merged and hardened; Flutter scaffold + Riverpod/go_router + mobile CI merged to `main`; Flutter Login/Signup + `AuthInterceptor` + secure token storage merged to `main`; `GET/PATCH /me` profile endpoint merged to `main` (PR #4); Flutter Profile screen + Edit Profile sheet merged to `main` (PR #5) |
...
| [Sprint 2](docs/16-development-roadmap.md#phase-1--sprint-2--user-profile--ai-onboarding) | User Profile & AI Onboarding | 🟡 In progress | Task 1 (`GET/PATCH /me` + validation) merged to `main` (PR #4, `5f7f706`). Task 2 (Flutter Profile screen) merged to `main` (PR #5, `cc2385f`) — Module 3 (User Profile) now Complete. Tasks 3–11 not started. |
...
Separately, [Sprint 2](docs/TASK_BREAKDOWN.md#sprint-2--user-profile--ai-onboarding)'s Task 1 (`GET/PATCH /me` endpoint + Form Request validation) and Task 2 (Flutter Profile screen + Edit Profile sheet) are both merged to `main` (squash-merges of [PR #4](https://github.com/karthik22feb/aesthetic-coach/pull/4), commit `5f7f706`, and [PR #5](https://github.com/karthik22feb/aesthetic-coach/pull/5), commit `cc2385f`), each approved by an independent reviewer before merge. Task 1 re-verified post-merge (144/144 Pest tests, Pint clean, `composer validate --strict` clean); Task 2 re-verified post-merge (`flutter analyze` clean, `dart format` clean, 90/90 tests passing, release APK build SUCCESS). **Module 3 (User Profile) is now Complete.** This was implemented ahead of strict roadmap order and does not change the fact that Task 20 is still the next task gating Module 2/Sprint 1 completion. Once Task 20 unblocks (a staging environment is provisioned), the next Sprint 2 candidates are Task 3 (basic Settings screen) and Task 9 (Pest tests: profile update + onboarding goal creation — the profile-update half is already substantially covered by the existing `ProfileTest.php`).
