...
| Repository / codebase | 🟡 In progress — Laravel backend scaffolded; register/login/logout merged and hardened; Flutter scaffold + Riverpod/go_router + mobile CI merged to `main`; Flutter Login/Signup + `AuthInterceptor` + secure token storage merged to `main`; `GET/PATCH /me` profile endpoint merged to `main` (PR #4); Flutter Profile screen + Edit Profile sheet merged to `main` (PR #5); basic Settings screen (theme, unit preference) merged to `main` (PR #6) |
...
[Phase 1 · Sprint 1](docs/16-development-roadmap.md#phase-1--sprint-1--infrastructure-authentication--project-setup) is underway: Infrastructure scaffolding is in progress, and the entire backend Authentication module (register/login/logout, refresh-token rotation, session/device management, Google/Apple Sign-In, email verification, password reset) is merged to `main`, tagged `v1.0.0-auth-foundation`, `v1.0.0-refresh-rotation`, `v1.0.0-session-management`, and `v1.0.0-auth-complete`. Flutter mobile foundation (Tasks 5–7: project scaffold, Riverpod/go_router, mobile CI) is merged to `main` (squash-merge of [PR #1](https://github.com/karthik22feb/aesthetic-coach/pull/1), commit `38f1da6`). Flutter Login/Signup, `AuthInterceptor`, and secure token storage (Tasks 17–19) are merged to `main` (squash-merge of [PR #2](https://github.com/karthik22feb/aesthetic-coach/pull/2), commit `f7a2580`); only Task 20 (staging E2E) remains unstarted. [Sprint 2](docs/16-development-roadmap.md#phase-1--sprint-2--user-profile--ai-onboarding)'s Task 1 (`GET/PATCH /me` endpoint), Task 2 (Flutter Profile screen), and Task 3 (basic Settings screen: theme, unit preference) have also landed ahead of strict sequence, merged to `main` (squash-merges of [PR #4](https://github.com/karthik22feb/aesthetic-coach/pull/4), commit `5f7f706`; [PR #5](https://github.com/karthik22feb/aesthetic-coach/pull/5), commit `cc2385f`; and [PR #6](https://github.com/karthik22feb/aesthetic-coach/pull/6), commit `450442b`) — this does not change Module 2/Task 20's blocked status. Module 3 (User Profile) is now **Complete**. Task 3 has no dedicated module in [IMPLEMENTATION_ORDER.md](docs/IMPLEMENTATION_ORDER.md#overall-development-order)'s 16-module list, so it's tracked at the Sprint Tracker level only, below. Sprint 2's Task 4 (`goals` table migration + Eloquent model, backing Module 4/AI Onboarding's "seeded `goals` row" output — the full `POST /goals` CRUD endpoint remains Sprint 4 scope) is also merged to `main` (squash-merge of [PR #8](https://github.com/karthik22feb/aesthetic-coach/pull/8), commit `7c5d12e`), approved by two independent reviewers distinct from the author (required for migration-touching changes) and re-verified post-merge (`composer validate --strict` clean, Pint clean, direct Tinker DB round-trip against the real local database confirmed enum casts/relations correctly) — the full Pest suite remains **not verified locally** (this bare-Windows environment has no Docker-Compose `mysql` hostname, which `phpunit.xml` forces; reproduces identically on unrelated pre-existing tests, so it is not a Task 4 regression). Module 4 (AI Onboarding) moves to **In progress**. See [Next Recommended Task](#next-recommended-task).
...
| [Sprint 2](docs/16-development-roadmap.md#phase-1--sprint-2--user-profile--ai-onboarding) | User Profile & AI Onboarding | 🟡 In progress | Task 1 (`GET/PATCH /me` + validation) merged to `main` (PR #4, `5f7f706`). Task 2 (Flutter Profile screen) merged to `main` (PR #5, `cc2385f`) — Module 3 (User Profile) now Complete. Task 3 (basic Settings screen: theme, unit preference) merged to `main` (PR #6, `450442b`). Task 4 (`goals` table migration + model) merged to `main` (PR #8, `7c5d12e`). Tasks 5–11 not started. |
...
| 3 | [User Profile](docs/IMPLEMENTATION_ORDER.md#3-user-profile) | ✅ Complete | 2 |
| 4 | [AI Onboarding](docs/IMPLEMENTATION_ORDER.md#4-ai-onboarding) | 🟡 In progress | 2 |
...
**Completed:** 1 of 16 (User Profile). **In progress:** 2 (Infrastructure, Authentication). **Remaining:** 13 not started — see [MODULE_DEPENDENCIES.md § Critical Path](docs/MODULE_DEPENDENCIES.md#critical-path) for which of these are schedule-critical.
...
Separately, [Sprint 2](docs/TASK_BREAKDOWN.md#sprint-2--user-profile--ai-onboarding)'s Task 1 (`GET/PATCH /me` endpoint + Form Request validation), Task 2 (Flutter Profile screen + Edit Profile sheet), and Task 3 (basic Settings screen: theme, unit preference) are all merged to `main` (squash-merges of [PR #4](https://github.com/karthik22feb/aesthetic-coach/pull/4), commit `5f7f706`; [PR #5](https://github.com/karthik22feb/aesthetic-coach/pull/5), commit `cc2385f`; and [PR #6](https://github.com/karthik22feb/aesthetic-coach/pull/6), commit `450442b`), each approved by an independent reviewer before merge. Task 1 re-verified post-merge (144/144 Pest tests, Pint clean, `composer validate --strict` clean); Tasks 2 and 3 re-verified post-merge (`flutter analyze` clean, `dart format` clean, 102/102 tests passing, release APK build SUCCESS). **Module 3 (User Profile) is now Complete.** Task 3 reuses Task 1/2's server-backed `unitPreference` field rather than a second local copy, and corrected two frozen docs (`docs/features/settings.md`, `docs/screens/settings.md`) that predated that decision. This was implemented ahead of strict roadmap order and does not change the fact that Task 20 is still the next task gating Module 2/Sprint 1 completion. Once Task 20 unblocks (a staging environment is provisioned), the next Sprint 2 candidates are Task 4 (`goals` table migration, no dependencies) and Task 9 (Pest tests: profile update + onboarding goal creation — the profile-update half is already substantially covered by the existing `ProfileTest.php`).
